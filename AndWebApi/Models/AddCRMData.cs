using System;
using System.Collections.Generic;
using System.Linq;
using System.Web;

namespace AndWebApi.Models
{
    public class AddCRMData
    {
        public string policyid { get; set; }
        public string businesstype { get; set; }
        public string pospid { get; set; }
        public string customergroupid { get; set; }
        public string customermemberid { get; set; }
        public string branchid { get; set; }
        public string companyid { get; set; }
        public string companybranchid { get; set; }
        public string productid { get; set; }
        public string producttype { get; set; }
        public string subproductid { get; set; }
        public string policyno { get; set; }
        public string issuedate { get; set; }
        public string isrenewable { get; set; }
        public string tenure { get; set; }
        public string startdate { get; set; }
        public string enddate { get; set; }
        public string registrationno { get; set; }
        public string engineno { get; set; }
        public string chasisno { get; set; }
        public string ccid { get; set; }
        public string manufacturingyear { get; set; }
        public string makeid { get; set; }
        public string modelid { get; set; }
        public string variantid { get; set; }
        public string dateofregistration { get; set; }
        public string noofseats { get; set; }
        public string idv { get; set; }
        public string rtoid { get; set; }
        public string zoneid { get; set; }
        public string ncb { get; set; }
        public string fueltypeid { get; set; }
        public string od { get; set; }
        public string tp { get; set; }
        public string addonprm { get; set; }
        public string netprm { get; set; }
        public string gst { get; set; }
        public string gstvalue { get; set; }
        public string totalpremium { get; set; }
        public string discount { get; set; }
        public string paymenttypeid { get; set; }
        public string cash { get; set; }
        public string bankid { get; set; }
        public string accountno { get; set; }
        public string amount { get; set; }
        public string chequeno { get; set; }
        public string chequedate { get; set; }
        public string bankid1 { get; set; }
        public string accountno1 { get; set; }
        public string amount1 { get; set; }
        public string chequeno1 { get; set; }
        public string chequedate1 { get; set; }
        public string bankid2 { get; set; }
        public string accountno2 { get; set; }
        public string amount2 { get; set; }
        public string chequeno2 { get; set; }
        public string chequedate2 { get; set; }
        public string posporreferenxe { get; set; }
        public string posporreferencecode { get; set; }
        public string brokerageodper { get; set; }
        public string brokeragetpper { get; set; }
        public string brokeragegst { get; set; }
        public string brokeagetotalamount { get; set; }
        public string feesodper { get; set; }
        public string feestpper { get; set; }
        public string feesgst { get; set; }
        public string feestotalamount { get; set; }
        public string createdby { get; set; }
    }
}